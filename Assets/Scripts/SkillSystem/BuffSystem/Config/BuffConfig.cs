...
[Serializable] [HideMonoScript] [CreateAssetMenu(fileName = "BuffConfig", menuName = "ScriptableObjects/BuffConfig", order = 0)]
public class BuffConfig : ScriptableObject
{
    ...
    [LabelText("触发间隔时间")]
    public int buffIntervalMS; // 如buff 每秒造成伤害 比如每秒掉血 每秒眩晕
    [LabelText("Buff持续时间(0表示一次 -1表示buff时间无限 直至战斗结束)")]
    public int buffDurationMS; // 如buff 持续多久 比如晕眩多久 比如持续掉血多久
    [LabelText("Buff类型")]
...
{
    [LabelText("无配置")] None,
    [LabelText("跟随目标")] TargetFallow,
    [LabelText("目标位置")] TargetPos,
}

[LabelText("Buff特效附着类型")]
